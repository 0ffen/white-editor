...
export function canToggle(editor: Editor | null, level?: Level | null, turnInto: boolean = true): boolean {
...
export function useHeading(config: UseHeadingConfig) {
  ...
  const canToggleState = level ? canToggle(editor, level) : (editor?.can().setParagraph() ?? false);
