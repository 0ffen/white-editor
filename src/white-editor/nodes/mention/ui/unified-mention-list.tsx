...
export const UnifiedMentionList = (props: UnifiedMentionListProps) => {
  ...
  if (allItems.length === 0) {
    return (
      <div
        ref={containerRef}
        className='we:bg-elevation-dropdown we:shadow-popover we:z-inline we:border-border we:relative we:flex we:max-h-52 we:min-w-26 we:flex-col we:overflow-y-auto we:rounded-md'
      >
      ...
      {mentionItems.length > 0 && (
        <>
          {showLabels && showPeopleLabel && (
            <div className='we:sticky we:top-0 we:z-inline we:bg-elevation-dropdown we:px-2 we:py-4 we:text-xs we:font-medium we:text-muted-foreground we:uppercase'>
              {peopleLabel}
            </div>
          )}
      ...
      {pageMentionItems.length > 0 && (
        <>
          ...
          {showLabels && showPagesLabel && (
            <div className='we:sticky we:top-0 we:z-inline we:bg-elevation-dropdown we:px-2 we:py-1.5 we:text-xs we:font-medium we:text-muted-foreground we:uppercase'>
              {pagesLabel}
            </div>
          )}
